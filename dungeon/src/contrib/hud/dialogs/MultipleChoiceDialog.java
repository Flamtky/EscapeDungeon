...
final class MultipleChoiceDialog {
  ...
  private static Group buildDialog(
    ...
    float optionWidth = optionWidthForFontSize(textFontSize);
    ...
    root.defaults().maxWidth(optionWidth);
    ...
    DialogScriptView scriptView = new DialogScriptView(script, textFontSize);
    ...
    Cell<Table> headerCell = root.add(header).maxWidth(optionWidth).padBottom(GAP);
    root.row();

    // Option rows
    final int[] selectedIndex = {-1};
    final List<Table> rowTables = new ArrayList<>();
    final List<Cell<Table>> rowCells = new ArrayList<>();
    ...
    for (int i = 0; i < allEntries.size(); i++) {
      ...
      float maxLabelWidth = optionWidth - OPTION_PAD * 2;
        ...
        new InputListener() {
          ...
          public boolean keyDown(InputEvent event, int keycode) {
            if (textFontSizeControls
              ...
              updateDialogWidths(headerCell, rowCells);
              root.pack();
              return true;
            }
  ...
  /**
   * Updates the header and option row width constraints after the dialog text font size changes.
   *
   * @param headerCell the root cell containing the script header
   * @param rowCells the root cells containing all option rows
   */
  private static void updateDialogWidths(Cell<Table> headerCell, List<Cell<Table>> rowCells) {
    float optionWidth = optionWidthForFontSize(DialogTextFontSizeControls.currentTextFontSize());
    headerCell.maxWidth(optionWidth);
    for (Cell<Table> rowCell : rowCells) {
      rowCell.maxWidth(optionWidth);
    }
  }

  /**
   * Calculates the total multiple-choice row width for a given dialog text font size.
   *
   * @param textFontSize base font size for the dialog script text
   * @return the desired option/header width in pixels
   */
  private static float optionWidthForFontSize(int textFontSize) {
    return DialogScriptView.fullTextWidthForFontSize(textFontSize) + MAIN_BOX_PAD * 2;
  }
