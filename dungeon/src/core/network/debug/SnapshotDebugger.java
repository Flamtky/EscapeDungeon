...
public final class SnapshotDebugger {
  ...
  private static final int UDP_MTU = NetworkConfig.SAFE_UDP_MTU;
  ...
  public static void logDelta(DeltaSnapshotMessage delta) {
    ...
    System.out.printf("Total: %d bytes %s%n", totalSize, "(EXCEEDS " + UDP_MTU + " MTU!)");
    ...
    for (Map.Entry<String, Integer> entry : sorted) {
      if (entry.getValue() > 0) {
        ...
        if (entry.getValue() > 20) {
          switch (entry.getKey()) {
            case "skillData" -> printSkillDataBreakdown(state);
  ...
  private static Map<String, Integer> measureEntityFields(EntityState state) {
    ...
    sizes.put("skillData", measureSkillData(state));
  ...
  private static int measureSkillData(EntityState state) {
    if (state.skillData().isEmpty()) return 0;
    return measureSize(state.skillData().get());
  }
  ...
  private static void printSkillDataBreakdown(EntityState state) {
    if (state.skillData().isEmpty()) return;

    SkillComponentData skillData = state.skillData().get();
    List<SkillData> skills = skillData.skills();

    if (skills == null || skills.isEmpty()) return;

    System.out.printf("       Skills (%d total):%n", skills.size());

    int shown = Math.min(3, skills.size());
    for (int i = 0; i < shown; i++) {
      SkillData skill = skills.get(i);
      int skillSize = measureSize(skill);
      String skillName = skill.name() != null ? skill.name() : "unknown";
      System.out.printf("         [%d] %s: %d bytes%n", i, skillName, skillSize);

      printSkillDataFieldBreakdown(skill, skillName);
    }

    if (skills.size() > shown) {
      int remaining = skills.size() - shown;
      int remainingSize = 0;
      for (int i = shown; i < skills.size(); i++) {
        remainingSize += measureSize(skills.get(i));
      }
      System.out.printf(
          "         ... and %d more skills (%d bytes total)%n", remaining, remainingSize);
    }
  }

  private static void printSkillDataFieldBreakdown(SkillData skill, String skillName) {
    int nameSize = skill.name() != null ? measureSize(skill.name()) : 0;
    int cooldownDurationSize = measureSize(skill.cooldownDurationMs());
    int remainingCooldownSize = measureSize(skill.remainingCooldownMs());
    int snapshotTimeSize = measureSize(skill.snapshotTimeMs());

    System.out.printf("           - name (%s): %d bytes%n", skillName, nameSize);
    System.out.printf(
        "           - cooldownDurationMs (%d): %d bytes%n",
        skill.cooldownDurationMs(), cooldownDurationSize);
    System.out.printf(
        "           - remainingCooldownMs (%d): %d bytes%n",
        skill.remainingCooldownMs(), remainingCooldownSize);
    System.out.printf("           - snapshotTimeMs: %d bytes%n", snapshotTimeSize);
  }
