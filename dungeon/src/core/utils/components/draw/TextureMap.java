...
public final class TextureMap extends HashMap<String, Texture> {
  ...
  public void putTexture(final IPath path, final Texture texture) {
    ...
    put(path.pathString(), configurePixelTexture(texture));
  }
  ...
  public TextureRegion cloneTexture(String path) {
    ...
    fbo.end();
    batch.dispose();
    TextureRegion region = new TextureRegion(configurePixelTexture(fbo.getColorBufferTexture()));
  ...
  private static Texture loadPMA(Pixmap pixmap) {
    ...
    try {
      ...
      return configurePixelTexture(new Texture(corrected));
    } finally {
      corrected.dispose();
    }
  }

  private static Texture configurePixelTexture(Texture texture) {
    if (texture == null) {
      return null;
    }
